...
function fmt(n) {
  ...
  const dec = Math.round(abs * 100) % 100 !== 0 ? 2 : 0;
  const s = new Intl.NumberFormat("es-AR", { minimumFractionDigits: dec, maximumFractionDigits: 2 }).format(abs);
  return (n < 0 ? "−" : "") + "$" + s;
}
function fmtMoneda(n, moneda) {
  if (moneda === "USD") {
    const abs = Math.abs(n);
    const dec = Math.round(abs * 100) % 100 !== 0 ? 2 : 0;
    return "U$S " + new Intl.NumberFormat("es-AR", { minimumFractionDigits: dec, maximumFractionDigits: 2 }).format(abs);
  }
  return fmt(n);
}
function fmtShort(n) {
