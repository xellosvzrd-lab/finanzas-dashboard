...
function renderPresupuesto() {
  ...
  const sueldo = dataMesDaniel
    .filter(t => t.tipo === "Ingreso" && CATS_INGRESO_REAL.includes(t.categoria)
             && (t.moneda || "ARS") === "ARS")
    .reduce((s, t) => s + Math.abs(Number(t.monto)), 0);
  // Para Ama: sueldoEfectivo incluye ingresos USD × MEP (no se restan gastos USD — el sueldo es lo que cobrás, no el saldo)
  ...
  if (sublSueldo) {
    sublSueldo.textContent = USUARIO.toLowerCase() === "ama" && tipoCambioMEP && ingresosUSDPres > 0
      ? `ARS + ${fmtMoneda(ingresosUSDPres,"USD")} × $${Math.round(tipoCambioMEP).toLocaleString("es-AR")} MEP`
      : "ingresos del mes";
  }
